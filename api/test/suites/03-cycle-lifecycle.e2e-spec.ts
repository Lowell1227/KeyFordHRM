...
describe('03-cycle-lifecycle', () => {
  ...
  it('指标确认→退回→确认 正常流转', async () => {
    ...
    const rejectRes = await app.http
      ...
      .send({ comment: '指标不合理' })
      .expect(200);
    expect(rejectRes.body.data.status).toBe('indicator_setting');
  ...
  it('完整 happy path：自评→主管评分→部门复核→HR校准→审批→公示→员工确认', async () => {
    ...
    const calibrateRes = await app.http
      ...
      .send({
        submit: true,
        calibrations: [{ taskId: task.id, calibratedGrade: 'B', calibrationNote: '校准说明' }],
      })
      .expect(200);
    expect(calibrateRes.body.data.submit).toBe(true);

    const taskAfterCalibrate = await app.prisma.assessmentTask.findUnique({ where: { id: task.id } });
  ...
  it('关周期：完成任务归档，无等级任务跳过，重复跑幂等', async () => {
    ...
    // 推到公示
    await app.http.post(`/api/v1/tasks/${task.id}/indicators/confirm`).set('Authorization', `Bearer ${empToken}`).expect(200);
    await app.http
      .post(`/api/v1/tasks/${task.id}/self-eval`)
      .set('Authorization', `Bearer ${empToken}`)
      .send({
        indicators: task.indicatorInstances.filter((i) => i.indicatorType !== 'veto').map((i) => ({ id: i.id, selfScore: 80 })),
        summary: {},
      })
      .expect(200);
    const managerScoreVersion = await app.prisma.assessmentTask.findUniqueOrThrow({
    ...
    await app.http
      .post(`/api/v1/cycles/${cycle.id}/calibration`)
      .set('Authorization', `Bearer ${hrToken}`)
      .send({ submit: true, calibrations: [{ taskId: task.id, calibratedGrade: 'B' }] })
      .expect(200);
    await app.http
      .post(`/api/v1/cycles/${cycle.id}/approval`)
      .set('Authorization', `Bearer ${approverToken}`)
    ...
    const archive = await app.prisma.performanceArchive.findFirst({ where: { cycleId: cycle.id, employeeId: employee.id } });
    expect(archive).toBeTruthy();
    expect(archive?.grade).toBe('B');

    // 幂等：再跑一次不报错、不新增 archive
    const archiveCountBefore = await app.prisma.performanceArchive.count({ where: { cycleId: cycle.id } });
    await scheduler.runAutoCloseCycles();
    const archiveCountAfter = await app.prisma.performanceArchive.count({ where: { cycleId: cycle.id } });
