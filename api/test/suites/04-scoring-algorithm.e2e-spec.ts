...
describe("04-scoring-algorithm", () => {
  ...
  async function launchCycleForEmployee(
    hrId: string,
    deptId: string,
    employeeId: string,
  ) {
  ...
  it("算分=82：A90 B80 C70 + bonus5 - penalty2", async () => {
    ...
    const indicators = task.indicatorInstances
      ...
      .map((i) => ({
        id: i.id,
        managerScore: scoreMap[i.name] ?? 80,
        managerComment: "评语",
      }));
    ...
    const gradeResult = await app.prisma.gradeResult.findUnique({
  ...
  it("豁免员工不参与算分与分布", async () => {
    ...
    const distribution = await app.http
      ...
      .set(
        "Authorization",
        `Bearer ${await login(app.http, { employeeNo: "HR001", password: "test123" })}`,
      )
      .expect(200);

    expect(distribution.body.code).toBe(0);
    expect(distribution.body.data.total).toBe(
      launchResult.totalTasks - launchResult.exemptedTasks,
    );
  });
  ...
  it("强制分布超上限返回 warning 但仍可提交", async () => {
    ...
    expect(
      res.body.data.warnings.some((w: string) => w.includes("A 等级比例")),
    ).toBe(true);
  });
});
