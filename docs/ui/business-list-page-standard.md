...
| 结构 | 适用场景 | 详情方式 | 已完成页面 |
|---|---|---|---|
| 管理台账型 `record` | 申诉、面谈、指标、模板、岗位目录 | 普通表单使用 `standard`，复杂编辑器使用 `wide` 右侧抽屉 | 申诉记录、绩效面谈、指标库、考核模板、岗位目录 |
...
| 主数据分栏型 `split-master` | 组织人员、花名册、先选范围再看记录 | PC 左侧范围、手机全屏范围抽屉 | 员工档案、组织架构 |
