...
包含查询、列表、分页、详情或维护动作的普通业务页面，应先选择以下三种结构之一。Dashboard、Reports、CycleWorkspace、团队绩效工作台等多面板复杂页面不强行套用。绩效校准保留专用分析区和操作逻辑，只统一最外层流程列表外壳。

| 结构 | 适用场景 | 详情方式 | 已完成页面 |
|---|---|---|---|
| 管理台账型 `record` | 员工档案、申诉、面谈、指标、模板、岗位目录 | 普通表单使用 `standard`，复杂编辑器使用 `wide` 右侧抽屉 | 员工档案、申诉记录、绩效面谈、指标库、考核模板、岗位目录 |
| 流程任务型 `workflow` | 转正、试用期历史、审批、复核、公示、改进计划 | 独立流程详情使用子路由驱动的 `workflow` 宽抽屉，手机全屏 | 转正管理/审批/我的申请、试用期考核三类列表、改进计划、部门复核、结果审批、结果公示、绩效校准、人事变更审核 |
| 主数据分栏型 `split-master` | 组织人员、先选范围再看记录 | PC 左侧范围、手机全屏范围抽屉 | 组织架构 |
...
截至 2026-09-15，批准方案中的阶段 1–4 已全部迁移。`ImprovementPlanListView`、转正审批/我的申请、试用期历史和部门复核使用列表内路由抽屉；原 `/improvement-plans/:id` 等详情 URL 仍可直接访问，并在父列表内打开。

以下页面明确不属于本轮普通列表模板迁移：工作台、绩效报表、周期工作区、个人/团队绩效办理工作区、目标地图及目标跟进画布。它们保持专用布局，不能据此把“全部阶段完成”理解为所有 Vue 页面都改造成列表模板。
