...
- [x] **Step 3: Implement the capability service**
...
- [x] **Step 4: Attach capabilities to login and `/auth/me`**
...
Assert that a dynamically assigned employee can reach the controller and its assigned cycle, while `canViewAll` can list all tasks but cannot approve or reject a task assigned to another user.

- [x] **Step 2: Verify RED**

Run the focused approval specs and confirm failures at the fixed role gate and write bypass.

- [x] **Step 3: Implement strict task ownership**

Remove the controller's fixed VP/chairman class gate. Include saved task roles in cycle visibility. Preserve read filtering, but require `approverId === currentUser.id` for every approval mutation regardless of system role or `canViewAll`.
...
- [x] **Step 3: Implement typed capability-aware access**
...
- [x] **Step 1: Write the failing visible-outcome test**
...
- [x] **Step 3: Implement compact identity display**
...
- [x] **Step 2: Review compatibility and authorization diff**
