...
export const TASK_STATUS_META: Record<TaskStatus, { label: string; type: string }> = {
  ...
  indicator_reviewing: { label: '待直属上级目标审核', type: 'warning' },
  ...
  manager_scoring: { label: '直属上级评分中', type: 'primary' },
  dept_review: { label: '部门复核中', type: 'primary' },
  hr_calibration: { label: '绩效校准中', type: 'primary' },
  approval: { label: '结果审批中', type: 'primary' },
