...
const statusMeta = computed(() => {
  ...
  return props.interview.status === 'pending'
    ? { label: '待面谈', type: 'warning' as const }
    : { label: '已记录', type: 'success' as const };
});
</script>
...
<template>
  <ChartCard class="interview-card">
    ...
    <template #extra>
      <el-tag v-if="statusMeta" :type="statusMeta.type" size="small">{{ statusMeta.label }}</el-tag>
